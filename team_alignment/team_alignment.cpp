...
namespace team {
    ...
    // void goal_cell(const std::vector<std::vector<cell>> &matrix,int &gi,int &gj){
        ...
        unsigned int* target_begin){
            ...
            switch (type) {
                case AlignmentType::global:
                    initialization = gap;
                    break;
                    // return needleman_wunsch(query_str, target_str, match, mismatch, gap, cigar, target_begin);
                case AlignmentType::local:
                    initialization = 0;
                    break;
                //     return smith_waterman(query_str, target_str);
                // case AlignmentType::semiGlobal:
                //     return gotoh(query_str, target_str);
                default:
            ...
            switch (type) {
                ...
                case AlignmentType::local:
                    ...
                    // std::cout<<"OVO:: "<<local_i<< " "<<local_j<<std::endl;

                    // for(int i=0; i<query_len+1; i++){
                    //     for (int j=0; j<target_len+1; j++){
                    //         std::cout<<"\t "<<m[i][j].cost ;
                    //         if(i>0 && j>0){ std::cout<<" "<<query_str[i-1]<<" "<<target_str[j-1];}
                    //     }
                    //     std::cout<<std::endl;
                    // }
                    ...
                    *cigar = result;
                    ...
                    // goal_cell(m,&gi,&gj); /* returns the desired final cell */
                    return( m[gi][gj].cost ); /* Steven Skiena, http://www.algorithm.cs.sunysb.edu/computationalbiology/*/
